...
class PutAwaySerialsListDialog_Return(private val context: Context, val material: MaterialReturn) :
    Dialog(context) {
    ...
    private fun setUpRecyclerView() {
        adapter = if (material.isBulk)
            ShowStringSerialsAdapter(listOf(material.returnedSerials[0]))
        else
            ShowStringSerialsAdapter(material.returnedSerials)
