...
class ReturnWorkOrderDetailsListFragment :
    ...
    lateinit var workOrder: WorkOrderReturn
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        if (arguments != null) {
            workOrder = WorkOrderReturn.fromJson(arguments?.getString(WORK_ORDER_KEY)!!)
            setUpRecyclerView()
        }
    }

    lateinit var adapter: ReturnWorkOrderDetailsAdapter
    private fun setUpRecyclerView() {
        adapter = ReturnWorkOrderDetailsAdapter(workOrder)
